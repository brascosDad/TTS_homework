...
def print_inventory inventory, inv_description
	inventory.each_with_index do |(key, value), index|
	puts "#{index+1}" + "." + "#{key}" + "-" + "#{value}"

	inv_description.each_with_index do |(key, value), index|
	puts "#{index+1}" + "." + "#{key}" + "-" + "#{value}"
end
end
...
# step 1. define the inventory hash and description hash
inventory = {
	"ketchup" => 3,
	"mayonnaise" => 10,
	"mustard" => 5,
...
inv_description = {
	"ketchup" => "sweet red sauce",
	"mayonnaise" => "fatty white goodness",
	"mustard" => "pardon me, do you have some salty goodness?",
	"hot_sauce" => "liquid fire"
}
...
while true
	...
	print_inventory (inventory, inv_description)
	...
	print_inventory (inv_description)
end
